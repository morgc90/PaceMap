...
function Avatar({ user, size = 36 }) {
  return <div className="avatar" style={{ width: size, height: size, fontSize: size * 0.35 }}>{user.avatar}</div>;
}
...
function DiscoverScreen({ onClubSelect }) {
  ...
  return (
    <div className="screen">
      <div className="top-bar"><div className="pm-logo">PM</div><div className="top-title">PACEMAP</div><div className="top-icon"><i className="ti ti-bell" aria-hidden="true" /></div></div>
...
function TripScreen({ onClubSelect }) {
  ...
  return (
    <div className="screen">
      <div className="top-bar"><div className="top-title">TRIP PLANNER</div><button className="new-trip-btn"><i className="ti ti-plus" aria-hidden="true" /> New trip</button></div>
        ...
        <div className="trip-header">
          ...
          <div className="trip-right">
            ...
            <button className={`privacy-btn ${isPublic ? "pub" : "priv"}`} onClick={() => setIsPublic(!isPublic)}>
              <i className={`ti ti-${isPublic ? "world" : "lock"}`} aria-hidden="true" /><span>{isPublic ? "Public" : "Private"}</span>
            </button>
          </div>
        </div>
...
function SocialScreen() {
  ...
  return (
    <div className="screen">
      <div className="top-bar"><div className="top-title">COMMUNITY</div><div className="top-icon"><i className="ti ti-user-search" aria-hidden="true" /></div></div>
...
function MessagesScreen() {
  ...
  if (activeConvo) return (
    <div className="screen messages-screen">
      ...
      <div className="msg-input-bar">
        ...
        <button className="send-btn" onClick={send}><i className="ti ti-send" aria-hidden="true" /></button>
      </div>
    </div>
  );
  ...
  return (
    <div className="screen">
      <div className="top-bar"><div className="top-title">MESSAGES</div><div className="top-icon"><i className="ti ti-edit" aria-hidden="true" /></div></div>
...
function ProfileScreen() {
  return (
    <div className="screen">
      <div className="top-bar"><div className="top-title">MY PACE</div><div className="top-icon"><i className="ti ti-settings" aria-hidden="true" /></div></div>
      <div className="profile-hero">
        ...
        <div className="profile-stats">
          {[["23","Clubs"],["8","Cities"],["142","Followers"],["89","Following"]].map(([v,l],i,a) => (
            <>
              <div key={l} className="pstat"><div className="pstat-val">{v}</div><div className="pstat-lbl">{l}</div></div>
              {i < a.length-1 && <div className="pstat-div" />}
            </>
          ))}
        </div>
      </div>
      ...
      {RUN_CLUBS.slice(0,3).map(c => (
        <div key={c.id} className="club-card" style={{margin:"0 16px 8px"}}>
          ...
          <div className="club-info"><div className="club-name">{c.name}</div><div className="club-meta"><i className="ti ti-map-pin" aria-hidden="true" /> {c.city}</div></div>
        </div>
      ))}
...
export default function App() {
  ...
  if (selectedClub) return (
    <div className="app"><div className="phone-frame"><div className="phone-screen">
      ...
      <div className="tab-bar">{TABS.map(t => <button key={t.id} className={`tab-item ${tab === t.id ? "active" : ""}`} onClick={() => { setSelectedClub(null); setTab(t.id); }}><i className={`ti ${t.icon}`} aria-hidden="true" /><span>{t.label}</span></button>)}</div>
    </div></div></div>
  );
  ...
  return (
    <div className="app"><div className="phone-frame"><div className="phone-screen">
      ...
      <div className="tab-bar">{TABS.map(t => <button key={t.id} className={`tab-item ${tab === t.id ? "active" : ""}`} onClick={() => setTab(t.id)}><i className={`ti ${t.icon}`} aria-hidden="true" /><span>{t.label}</span></button>)}</div>
    </div></div></div>
  );
}
